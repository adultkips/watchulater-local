(() => {
  ...
  async function prefill(){
    ...
    const data = await fetch('get_roulette_edit.php?id=' + encodeURIComponent(editId), { cache: 'no-store' }).then(r => r.json());
    if (data && !data.error) {
      document.getElementById('page-title').textContent = 'Edit roulette';
      ...
      mark(moviesWrap, data.movie_source);
      mark(showsWrap, data.show_source);
      updateTypeLock();
      syncCoverSelection();
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      movie_source: movieKeys.join(','),
      movie_title: movieTitles.join(','),
      show_source: showKeys.join(','),
      show_title: showTitles.join(','),
    ...
    const res = await fetch('save_roulette.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...csrfHeaders },
    ...
    if (res.ok && data.success) {
      window.location.href = 'roulettes.php';
    } else {
  ...
  loadSections().then(prefill).then(() => {
    syncCoverSelection();
    showStep(1);
  });
})();
