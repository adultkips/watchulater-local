(() => {
  ...
  const addTile = document.getElementById('add-battle');
  ...
  function makeCard(item) {
    ...
    link.href = 'battle-view.php?id=' + encodeURIComponent(item.id);
    link.setAttribute('aria-label', 'Open battle');
    ...
    meta.className = 'meta';
    const sections = (item.source_title || '').trim();
    ...
    edit.addEventListener('click', (e) => {
      e.stopPropagation();
      window.location.href = 'create_battle.php?id=' + encodeURIComponent(item.id);
    });
  ...
  function renderSections(list, selected = []) {
    ...
    list.forEach(i => {
      const src = (i.source_title || '').split(',').map(s => s.trim()).filter(Boolean);
  ...
  resetBtn?.addEventListener('click', async () => {
    ...
    if (!confirm(`Delete ${visibleIds.length} battle(s)? This cannot be undone.`)) return;
    await fetch('delete_all_battles.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...csrfHeaders },
